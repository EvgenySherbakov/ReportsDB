...
OBJECTS = {
    ...
    "v_report_twin": "Двойники отчёта: строка на «отчёт × область сравнения» "
                     "(scope = NETWORK — другие ТС, PLANT — другие заводы своей "
                     "ТС). Тёзки и ближайший по набору таблиц, со сходством. "
                     "Порог уникальности задаёт читатель, в витрине его нет.",
    "v_network_report_twin": "Отчёт сети целиком: строка на «ТС + наименование», "
                             "заводы схлопнуты. Есть ли этот отчёт в других ТС.",
    "v_catalog_overview": "Свод по папкам каталога.",
    "v_schema_overview": "Свод по схемам БД.",
    "v_network_overview": "Сравнение РЦ: отчёты, объём базы завода, сколько из него под отчётами и сколько без них.",
}
...
COLUMNS = {
    ...
    "kind_source": "Откуда известен тип: «колонка», «маска», «файл размеров», "
                   "«по умолчанию».",
    "schema_source": "Откуда известна схема: «ссылка» (была в тексте), «файл "
                     "размеров» (восстановлена по единственному совпадению "
                     "имени), «не определена».",
    ...
    "is_orphan": "Объект не используется ни одним отчётом — на него не ссылается ни одна строка выгрузки.",
    "is_parsed_ok": "Схема известна — своя или восстановленная; иначе схема "
                    "«(unknown)». Подробности в schema_source.",
    ...
    "plant": "Завод.",
    "scope": "Область сравнения: NETWORK — с отчётами других ТС, PLANT — с "
             "другими заводами своей ТС.",
    "best_jaccard": "Сходство набора таблиц с ближайшим отчётом на другой "
                    "стороне: 1.0 — наборы совпадают. NULL — такого отчёта нет "
                    "(или у отчёта нет таблиц).",
    "best_shared_tables": "Сколько таблиц общих с этим ближайшим отчётом.",
    "name_twin_count": "Сколько сетей (в области NETWORK) или заводов своей ТС "
                       "(в области PLANT) держат отчёт с тем же наименованием.",
    "counterparts_compared": "С каким числом сетей или заводов было что "
                             "сравнивать. 0 — сравнивать не с чем.",
