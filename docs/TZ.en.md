...
- **№1 is exactly the sizes file, no more and no less.** The catalog of tables
  ...
  **nothing beyond it does**: objects from reports without a measurement are not
  ...
  `report_count` and `rc_report_count` columns in №1 are **informational**: they
  answer "is the table used", but they do not select rows. Their difference
  matters: `report_count` counts reports across the whole base,
  `rc_report_count` only the reports of the same plant as the row. **DC metrics
  count by `rc_report_count`**: a table reached by a neighbouring plant's report
  still lies idle on this one, and the base-wide count would understate "not
  used" roughly twofold at 70% report overlap between networks. Metrics counting
  tables (for example "not used by any report") use
  `COUNT(DISTINCT full_name)`: counting rows would inflate them by the number of
  plants the table lives on. Being unused is decided **by the selection**, not by
  the row: "not used" means "no report of the selected DCs touches it", otherwise
  with all plants selected the metric would answer a different question — "unused
...
- **A search may hold several values** — separated by a comma, a semicolon or a
  newline; a row enters the result if it matches at least one of them (OR, not
  AND: an intersection across different tables is always empty). A space is not
  a separator — report names consist of several words. The widget is
  `st.text_area`, not `st.text_input`: a list from Excel cannot be pasted into a
  single-line field, the browser strips newlines by specification and the column
  merges into one word. The search is a substring match (`regex=False`): the
  customer pastes real names, `[dbo].[Orders]` read as a regex would find
  entirely the wrong thing, and a lone `*` used to crash the page. Values that
  matched nothing are listed in the caption — when pasting a list that is the
  main thing to know.
...
1. **Overview** — the main dashboard, everything scoped to the selected DC, the
   ...
   **not** obey the DC selector in the header: it is a standalone catalog of
   ...
   "used by reports" / "unused"), search by name. KPIs: tables loaded in total
   ("table + plant" rows), unique tables, total volume, how many tables no
   report of the selected DCs uses. Under the table
...
| 2026-08-02 | **The search accepts several values at once.** The customer arrives with a list of tables — "show me these twenty" — while the field matched exactly one substring. Values are now separated by a comma, a semicolon or a newline, and a row enters the result if it matches at least one: this is parsing a list, not intersecting conditions, which across different tables would always be empty. A space was deliberately not made a separator — report names consist of several words, and «Продажи за месяц» would fall apart into three useless fragments. The widget had to change from `st.text_input` to `st.text_area`: a list from Excel cannot be pasted into a single-line field — the browser strips newlines by specification, and a check confirmed that a column of three tables merges into one word. The height is minimal so as not to eat the screen where the row breakdown must fit under the table. Values that matched nothing are listed in the caption — when pasting a list that is the main thing: otherwise it is unclear whether the name is absent from the database or simply mistyped. A long-standing bug was fixed on the way: `str.contains` parsed the query as a regular expression, so `[dbo].[Orders]` matched rows containing the letters d, b, o, and a lone `*` crashed the page with "nothing to repeat" — now `regex=False`. The same applies in the colleague export. |
| 2026-08-02 | **Exact matching in the search, and all reports for a whole list of tables at once.** The customer pasted a list of tables and got extra rows: the search matched part of a name, and `itm.log` is contained in `dbo.catalog`, in `wh1.errlog` and in `ssaadmin.vtmuserlogins`. An "Exact match" checkbox was added: names are compared in full. A name without a schema also counts as a match (`TRIP_LOG` finds `sdd.trip_log`) — in exports half the names arrive with a schema and half without, and making the customer split the list by hand is not acceptable. The mode does not switch itself by the number of values: implicit behaviour of "one value this way, several that way" would guess right almost always, and in the remaining cases would look like a malfunction. Instead, with several values in part-of-a-name mode a hint appears using the same example as the complaint. Second: the click-a-row breakdown answers a question about one table, while people arrive with a list — page №2 "By tables" gained an "All reports for the found tables" block with "table + report" relations across the whole selection and CSV export. In the colleague export, exact matching is one toggle in the header next to the DC selector rather than a checkbox on every tab. |
| 2026-08-02 | **The "tables → reports" export as an Excel workbook with two sheets.** CSV is fine for dragging rows into another program, but the customer works in Excel and expects a ready workbook. The "Отчёты" sheet holds one row per report: how many of its tables the query matched and which ones, separated by `;`; the "Таблицы и отчёты" sheet holds the same pairs as on screen. The `build_workbook()` helper in `app/_shared.py` assembles a workbook from a "sheet name → frame" mapping and suits any page: it renames columns through the shared `LABELS` dictionary (people open the file, and `table_full_name` in a header tells them nothing), drops surrogate keys, fits column widths to the content and freezes the header row. A sheet name is truncated to 31 characters and cleared of `: \ / ? * [ ]` — Excel rejects such names and opens the file with an error. Numbers stay numbers, so formulas and sorting work immediately. Written through `openpyxl`, already a dependency: it reads the source `.xlsx` files and now writes them too. |
...
| 2026-08-07 | **The query text reached a fifth of the reports because the "breakdown" was decided by the file header rather than by the row.** The customer asked why one plant had a query on only 18% of its reports and suspected the catalogs. The catalogs were innocent: a reproduction on a two-plant database showed that blanking the catalog drops matching from 99% to 98% — only true namesakes within a plant are lost. The culprit was `has_rc` in `_load_report_sql`: the "this file is split by plants" flag was computed once, from the presence of the ТС and Завод columns in the header. The moment those columns are unusable — empty because of vertically merged cells (the value sits in the first row of the range, the rest are blank), or the plant code written differently from the main file (`0358` versus `358`) — both precise tiers fall away, and the third one, the bare name, accepts only names unique across the whole database when `has_rc = True`. On a database of several plants with shared names that is exactly 18%: the share of names occurring nowhere else. Checked on synthetic data where the plants share 82% of the names — it came out at 17%, matching the complaint. An extra insult: a file **without** ТС and Завод columns gave 100%, so having unusable columns was worse than having none. Fixed in three parts. First: the breakdown is decided per row — empty ТС and Завод cells mean no information about a plant, and such a row behaves like a row of a file without those columns, landing on every namesake (empty and merged cells → 100%). Second: a ТС + Завод pair absent from the report catalog is recorded separately, but the behaviour is unchanged — matching by a unique name is kept so the fix takes nothing away; instead the "Load data" page names the pairs from the file next to the pairs from the catalog and says outright that this is what "a fifth of it matched" looks like. Third: `executemany` with an empty list crashes DuckDB, so "no row landed" used to abort the entire database build over an optional file — now it is a diagnosis, not an accident. A `MatchReport` appeared — a breakdown of unmatched rows by reason (name absent from the catalog / name present but not on those ТС and Завод / namesakes with a non-matching catalog); it is filled for the statistics file too, where handing a row to namesakes is conversely forbidden: statistics belong to a site, while the query text describes the report's definition. The change is in the ETL only, `SCHEMA_VERSION` was not raised, but the data does need reloading — otherwise the queries stay where they were. |
| 2026-08-07 | **One invisible letter left the reports without queries, and there was nothing to name it with.** The breakdown by reason from the previous entry paid off immediately: the customer sent ««ТСX / 358» in the file, «ТСХ / 358» in the catalog» — two spellings differing in the last letter, Cyrillic «Х» (U+0425) in the catalog against Latin «X» (U+0058) in the file. On screen it is one and the same inscription. An exact comparison separated them, the pair counted as absent, and matching fell back to "a name unique in the database": 703 rows out of 1614 landed nowhere. Such a thing cannot be spotted by eye, and the earlier "bring the spelling into line" message was a mockery — the customer saw identical text in both files. `loose_key()` was added to `normalize.py`: letter case, non-breaking and repeated spaces, zero-width ones, an em dash versus a hyphen, «ё» versus «е», and a table of visually identical Cyrillic and Latin letters. The key is lowercased BEFORE substitution, so the table holds only lowercase pairs and the uppercase ones («В» and «B», «Н» and «H») reduce to them by themselves. Digits are deliberately absent from the table: Cyrillic «З» instead of «3» is a typo too, but that one is better named than silently corrected. The report-catalog indexes moved into `_ReportIndex`: the exact keys and the loose ones lie side by side, and the loose one is checked ONLY after the exact one found nothing — there is a dedicated test with both spellings of the network in the catalog, where the row must go to its own. The tolerance extends to the report name and the catalog path as well: the files are exported by different systems, and a non-breaking space in a name broke matching just as quietly. The load now reports how many rows matched "by looks" and names **the position and the code point of the differing character** — `spell_out_difference()` prints "позиция 3: в файле латинская «X» (U+0058), в каталоге кириллическая «Х» (U+0425)", and that hint is enough to find the typo by searching the file itself. Verified in the browser on demo data with Cyrillic «С» swapped for Latin «C» in the network name: 66 rows out of 67 matched "by looks", the message named both spellings and the code point, and the sixty-seventh is a deliberately non-existent report. `SCHEMA_VERSION` was not raised, but the data does need reloading. |
| 2026-08-07 | **"Not used by any report" answered the wrong question: it counted across the whole base rather than per DC.** The customer asked to check the metric — and it really did count blindly. `report_count` in `v_tables_catalog` is a correlated subquery over `bridge_report_table` with no plant, i.e. identical for every row of the same table. But a row of the view is a "table + plant" pair, and page №1 lives in the "DC analytics" section with network and plant filters: there "not used" means "no report of THIS plant touches it". A table reached by a neighbouring plant's report looked used under the base-wide count, though it lies idle on this plant. On demo data the discrepancy is twofold: 98 against 197 tables on the first plant; at the 70% report overlap between networks the customer mentioned, nearly everything came out "used". An `rc_report_count` column was added — the reports of the same DC as the row; `report_count` was kept beside it, because "zero for the plant and non-zero for the base" is a meaningful case of its own — "the table is kept for another plant's report" — and it got a ready query on the ad-hoc SQL page. The same count fixed `used_table_count`/`used_mb` in `v_network_overview`: there EVERY row is a DC, so the base-wide count was even more clearly wrong, and the "Networks and plants" figures disagreed with №1. A dedicated test reconciles both views row by row. The second subtle point: being unused is decided by the SELECTION, not by the row — a per-row count would answer "unused on at least one plant", and with all plants selected the metric would lie again. Also at the customer's request: the "table + plant rows" metric is now called "tables loaded in total" (what it counts is in the tooltip), "Database queries" became "Database queries from SSRS", and the tabs got a visible look — they differed only by a thin line under the active one, and the customer had no idea "By reports" and "By tables" switched; the Streamlit selectors had to be checked against the actual DOM (`div[data-testid="stTab"]`, not `button[data-baseweb="tab"]` — this version differs). `SCHEMA_VERSION` was raised to 14: views were edited, a data reload is mandatory. |
